...
- UNHEALTHY OWNER + BLOCKED FRONTIER (reassign the blocker first): If TODO tasks are blockedBy an in_progress/pending task whose owner is unhealthy/stale/offline, and healthy idle teammates exist, IMMEDIATELY task_set_owner that blocker to a healthy teammate and task_start them. Do not leave the whole board waiting on one red agent while others sit idle — that is exactly the failure mode the user hates.
