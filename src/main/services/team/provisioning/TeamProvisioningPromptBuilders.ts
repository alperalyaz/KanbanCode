...
export function buildPersistentLeadContext(opts: {
  ...
  const teamConstraint = !isSolo
    ? `\n- BOARD PLAN FIRST (MANDATORY for teams with teammates): After you decompose ANY actionable user request (implementation, research, review, debugging, git ops, analysis — not pure chat), create EVERY identified work item as a pending board task via task_create (startImmediately: false) with owners before anyone begins execution.` +
...
- REPORT THE BOARD ACCURATELY — match what the user actually sees (do NOT say "TODO is full" when TODO is empty): The user is looking at the SAME kanban board you are, with real columns — TODO holds only PENDING (not-yet-started) tasks, IN PROGRESS holds started/in-progress tasks, DONE holds finished ones. After BOARD PLAN FIRST seeding, TODO MUST show leftover backlog; only the active slice belongs in IN PROGRESS. When a task is created and then started, it LEAVES the TODO column and moves to IN PROGRESS — so if you create and immediately start everything in one turn, TODO will look empty even though work is running. That empty-TODO outcome is FORBIDDEN after a seeding turn. Prefer seeding pending tasks first, then starting only what begins now (at most one per idle teammate). Never tell the user "TODO dolu / TODO is full" (or similar) as a loose shorthand for "I queued up work", because the user then looks at an empty TODO column and is confused about whether you see a different board. Describe the ACTUAL state in the ACTUAL column names: e.g. "8 görev oluşturdum — 5'i TODO'da bekliyor, 3'ü IN PROGRESS'te @Frodo/@Sam/@Merry'de" ("created 8 tasks — 5 pending in TODO, 3 in progress with Frodo/Sam/Merry"). Your words about the board must match the columns the user is staring at, task IDs and all.
