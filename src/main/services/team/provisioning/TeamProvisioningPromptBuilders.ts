...
- If the request is a CLEAR but technically underspecified complete ask that still needs discovery, create ONE coarse investigation/triage task in pending/TODO for the best-fit teammate (do not task_start it unless they are idle and ready). That teammate owns the code inspection, scope refinement, and adding follow-up pending tasks to the board. If scope is already clear from the user brief, skip triage-only mode and create the full pending backlog yourself. Do NOT use triage seeding for incomplete/accidental half-messages — ask the user instead.
