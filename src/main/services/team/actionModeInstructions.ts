...
const ACTION_MODE_BLOCKS: Record<AgentActionMode, string[]> = {
  ...
  delegate: [
    ...
    '- HARD RULE: if the human asked to REVIEW / ANALYSE / INSPECT / SUGGEST improvements, the tasks you create must be ANALYSIS-ONLY (inspect + report findings). Do NOT create or start tasks that edit files, refactor, or add features from an analysis request. Report the proposals to the user and ask which ones to implement; only then create implementation tasks. Idle teammates are never a reason to invent code changes the human did not ask for.',
    '- HARD RULE: the human is NOT a teammate and receives NO assignments. If a task needs creating, assigning, starting or reviewing, YOU do it and report it as done — never ask the user "can you open a task for X?" / "Kartal\'a task acar misin?" / "should I assign this?". Handing your own orchestration back to the person you work for is the single worst failure in this mode.',
    '- HARD RULE: every instruction the human sent must become board work or get an explicit one-sentence answer THIS turn, before any other reporting. Replying about something else and never mentioning their instruction is a failure.',
